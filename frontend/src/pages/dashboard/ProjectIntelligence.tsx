...
import { Activity, AlertTriangle, ArrowLeft, Bot, Clock, IndianRupee, PlusCircle, Send, ShieldAlert } from 'lucide-react';
import { Link, useParams } from 'react-router-dom';
import { aiApi, projectApi } from '../../services/api';
import type { ProjectDetail, ProjectHistoryItem, RiskPredictionResult } from '../../services/api';
...
const ProjectIntelligence = () => {
  ...
  const fetchDetails = () => {
    projectApi.detail(id).then(({ data }) => setProject(data)).catch(() => setError('Unable to load project data.'));
  };
    ...
    <div className="flex items-center justify-between">
      ...
      <div className="flex items-center gap-2.5">
        <button
          ...
          className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl inline-flex items-center gap-1.5 shadow-xs transition-all cursor-pointer"
        >
          <PlusCircle className="w-3.5 h-3.5" /> + Add Monthly Update
        </button>
        <Link to={`/dashboard/projects/${encodeURIComponent(project.project_code)}/risk`} className="px-4 py-2 bg-government-blue hover:bg-blue-800 text-white text-xs font-bold rounded-xl inline-flex items-center gap-1.5 shadow-sm transition-all">
          <ShieldAlert className="w-3.5 h-3.5 text-amber-400" /> Deep Risk Assessment
        </Link>
      </div>
    </div>
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-3">
          <h1 className="text-2xl font-bold text-black">{project.project_name}</h1>
          <span className="px-2.5 py-1 bg-slate-200 text-slate-700 text-xs font-bold rounded-md">{project.project_code}</span>
        </div>
      </div>
      <p className="text-sm text-slate-500 mt-1">{project.ministry || 'No ministry available'} · {project.agency || 'No agency available'} · {project.state || 'No state available'}</p>
    </div>
